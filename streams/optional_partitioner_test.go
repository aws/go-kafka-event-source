...
func TestOptionalPartitioner(t *testing.T) {
	...
	p := partitionerA.Partition(assigned.asKgoRecord(), 100)
	if p != 11 {
		t.Errorf("Incorrect partition. actual: %d, expected: %d", p, 11)
	}

	p = partitionerA.Partition(unassigned.asKgoRecord(), 100)
	if p != 1 {
		t.Errorf("Incorrect partition. actual: %d, expected: %d", p, 1)
	}

	p = partitionerB.Partition(unassigned.asKgoRecord(), 100)
	if p != 2 {
		t.Errorf("Incorrect partition. actual: %d, expected: %d", p, 2)
	}

	p = defaultPartitioner.Partition(unassigned.asKgoRecord(), 100)
	if p != 10 {
